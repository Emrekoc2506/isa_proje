...
export default function DashboardPage({ activeTab = "overview" }) {
  ...
  const [addingWishId, setAddingWishId] = useState(null);
  const [addedWishId, setAddedWishId] = useState(null);
  ...
  const getPasswordStrength = useCallback((pass) => {
    ...
    if (score <= 4)
      return { score, text: "Orta Derece Şifre", color: "#f39c12" };
  ...
  const handleEmailChange = async (e) => {
    ...
    if (newEmail.toLowerCase() === (user?.email || "").toLowerCase()) {
      setEmailError(
        "Yeni e-posta adresi mevcut e-posta adresinizle aynı olamaz.",
      );
      return;
    }
            ...
            {NAV_ITEMS.map(({ id, label, icon: Icon }) => (
              <button
                ...
                onClick={() => {
                  setActive(id);
                  navigate(
                    `/${id === "overview" ? "panel" : id === "wishlist" ? "favorilerim" : id === "profile" ? "profilim" : id === "orders" ? "siparislerim" : id === "addresses" ? "adreslerim" : id === "messages" ? "panel" : "ayarlar"}`,
                  );
          ...
          <div className={styles.topBarActions}>
            <div style={{ position: "relative" }}>
              ...
              >
                <FiBell />
                {unreadCount > 0 && (
                  <span className={styles.notifBadgeDashboard}>
                    {unreadCount}
                  </span>
                )}
              </button>
                ...
                <div className={styles.welcomeBanner}>
                  <div>
                    ...
                    <p className={styles.welcomeSub}>
                      Mistik alışveriş deneyimine hazır mısın?
                    </p>
                  </div>
                    ...
                    <>
                      {ordersList.map((order) => (
                        <OrderRow
                          key={order.id}
                          order={order}
                          initialOpen={routeOrderId === order.id}
                        />
                      ))}
                      {ordersList.length === 0 && (
...
function OrderRow({ order, compact, initialOpen = false }) {
  ...
  const cfg = {
    label: order.status || "Sipariş Verildi",
    color: "var(--gold)",
  };
              ...
              >
                ...
                <div style={{ display: "flex", gap: 20 }}>
                  ...
                  <span
                    ...
                    style={{ color: "var(--gold-light)", fontWeight: 500 }}
                  >
                    {item.price}
                  </span>
                </div>
